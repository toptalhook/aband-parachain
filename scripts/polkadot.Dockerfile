...
WORKDIR /testnet/substrate-parachain-PoS-template
RUN cargo build --locked --release
WORKDIR /testnet/polkadot
RUN cargo build --locked --release
...
ENTRYPOINT ["/usr/local/bin/polkadot-launch /data/config.json"]
